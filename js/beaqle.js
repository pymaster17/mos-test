...
// Audio pool object. Creates and manages a set of <audio> tags.
    ...
    AudioPool.prototype.addAudio = function(path, ID){
        ...
        // Only request CORS mode when Web Audio is actually active. Plain
        // <audio> playback works fine cross-origin and is more permissive.
        if (isCrossOrigin && this.waContext !== false) {
...
//        this.TestState.Runtime.forEach(function(element, index, array){array[index] = 0});
    ...
    // ###################################################################
    // Preload all audio files for the current test run
...
MushraTest.prototype.createTestDOM = function (TestIdx) {
    // 清除旧的测试表格（原代码已有逻辑，若存在则移除旧表格）
    if ($('#TableContainer > table')) {
        $('#TableContainer > table').remove();
    }

    // 创建随机文件映射（原代码已有逻辑，此处保留）
    if (!this.TestState.FileMappings[TestIdx]) {
        this.createFileMapping(TestIdx);
    }
...
PrefTest.prototype.createTestDOM = function (TestIdx) {
        ...
        // 添加参考文本显示逻辑，先获取配置文件里的转录文本信息并展示
        var headRow = tab.insertRow(0);
        var textCell = document.createElement('td');
        textCell.colSpan = 4;
        textCell.innerHTML = this.TestConfig.Testsets[TestIdx].transcribe;
        headRow.appendChild(textCell);
...
//     // 添加参考文本显示逻辑，先展示基本的测试名称、编号信息
//     var headRow = tab.insertRow(0);
//     var textCell = document.createElement('td');
//     textCell.colSpan = 4;
//     textCell.innerHTML = "参考文本：<br>测试名称 - " + this.TestConfig.Testsets[TestIdx].Name + "<br>测试编号 - " + this.TestConfig.Testsets[TestIdx].TestID;
//     // 接着添加转录文本信息展示
//     textCell.innerHTML += "<br><br>参考转录文本：<br>" + this.TestConfig.Testsets[TestIdx].transcript;
//     headRow.appendChild(textCell);
...
//     var cell = [];
//     fileID = this.TestConfig.Testsets[TestIdx].Files["A"];
//     cell[0] = row.insertCell(0);
//     cell[0].innerHTML = '<button id="play' + fileID + 'Btn" class="playButton" rel="' + fileID + '">A</button>';
//     this.addAudio(TestIdx, fileID, fileID);

//     // 添加 B 选项相关元素
//     fileID = this.TestConfig.Testsets[TestIdx].Files["B"];
//     cell[1] = row.insertCell(1);
//     cell[1].innerHTML = '<button id="play' + fileID + 'Btn' +
//         '" class="playButton" rel="' + fileID + '">B</button>';
//     this.addAudio(TestIdx, fileID, fileID);
...
//     cell[1].innerHTML = "<input type='radio' name='ItemSelection' id='selectB'/>";
//     cell[2] = row.insertCell(2);
//     cell[2].innerHTML = "<input type='radio' name='ItemSelection' id='selectEqual'/> <span style='margin-left: 5px;'>（表示无偏好，A和B相等）</span>";
//     cell[3] = row.insertCell(3);
//     cell[3].innerHTML = "Please select the item which you prefer or choose Equal if you have no preference!";
...
PrefTest.prototype.readRatings = function (TestIdx) {
    if (this.TestState.Ratings[TestIdx] === "A") {
        ...
        $("#selectB").prop("checked", true);
    } else if (this.TestState.Ratings[TestIdx] === "Equal") {
        $("#selectEqual").prop("checked", true);
    }
}
...
PrefTest.prototype.saveRatings = function (TestIdx) {
    if ($("#selectA").prop("checked")) {
        ...
        this.TestState.Ratings[TestIdx] = "B";
    } else if ($("#selectEqual").prop("checked")) {
        this.TestState.Ratings[TestIdx] = "Equal";
    }
}
...
MosTest.prototype.createTestDOM = function (TestIdx) {
    ...
    $('.rateSlider').each( function() {
        $(this).slider({
                ...
                min: mosConf.RateMinValue,
                max: mosConf.RateMaxValue,
                step: 0.5,
                animate: false,
                orientation: "horizontal"
        });
